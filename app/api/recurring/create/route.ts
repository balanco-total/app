...
export async function POST(request: Request) {
  ...
  const { description, amount, category_id, financial_account_id, day_of_month, start_year_month } =
  ...
  if (!Number.isInteger(dom) || dom < 1 || dom > 31)
    return NextResponse.json({ error: 'Dia do mês inválido (1–31).' }, { status: 400 })

  const startYm = String(start_year_month ?? '')
  if (!/^\d{4}-\d{2}$/.test(startYm))
    return NextResponse.json({ error: 'Mês de início inválido.' }, { status: 400 })
  ...
  if (!checkRateLimit(`recurring:${user.id}`, 20, 60 * 1000))
  ...
  if (error) return NextResponse.json({ error: 'Erro ao criar despesa recorrente.' }, { status: 500 })
