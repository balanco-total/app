...
export function BarTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-white dark:bg-dm-card border border-gray-200 dark:border-white/[0.08] rounded-lg shadow-lg p-3 text-sm">
      <p className="font-semibold text-gray-700 dark:text-dm-muted mb-1">{label}</p>
...
export function PieTooltip({ active, payload }: any) {
  ...
  return (
    <div className="bg-white dark:bg-dm-card border border-gray-200 dark:border-white/[0.08] rounded-lg shadow-lg p-3 text-sm">
      <p className="font-semibold text-gray-700 dark:text-dm-muted">{payload[0].name}</p>
      <p className="font-bold" style={{ color: payload[0].payload.fill }}>{fmt(payload[0].value)}</p>
      <p className="text-gray-500 dark:text-dm-muted">
