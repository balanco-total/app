  ...
  return (
    <div className="bg-white dark:bg-dm-card rounded-2xl shadow-lg p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <h2 className="text-xl font-bold text-gray-800 dark:text-dm-text">Últimos lançamentos</h2>
        <div className="flex flex-wrap gap-2">
          {financialAccounts.length > 0 && (
            <select
              value={filterAccountId}
              onChange={e => changeFilter(setFilterAccountId, e.target.value)}
              className="text-sm border border-gray-200 dark:border-white/[0.14] rounded-lg px-3 py-1.5 text-gray-700 dark:text-dm-muted bg-white dark:bg-dm-field focus:outline-none focus:ring-2 focus:ring-gray-300 sm:w-44"
            >
          ...
          <select
            ...
            className="text-sm border border-gray-200 dark:border-white/[0.14] rounded-lg px-3 py-1.5 text-gray-700 dark:text-dm-muted bg-white dark:bg-dm-field focus:outline-none focus:ring-2 focus:ring-gray-300 sm:w-48"
          >
      ...
      <div className="space-y-2">
        {paginated.map(exp => {
          ...
          const categoryPill = (
            <div className="relative">
              ...
              {editingCategoryId === exp.id && (
                <div className="absolute right-0 top-full mt-1 z-20 bg-white dark:bg-dm-card rounded-xl shadow-xl border border-gray-100 dark:border-white/[0.08] py-1 min-w-[160px]">
                  {categories.map(cat => (
                    <button
                      key={cat.id}
                      onClick={() => {
                      ...
                      className={`w-full flex items-center gap-2 px-3 py-2 hover:bg-gray-50 dark:hover:bg-dm-field text-sm text-left transition ${
                        cat.id === exp.category_id ? 'font-semibold text-gray-900 dark:text-dm-text' : 'text-gray-700 dark:text-dm-muted'
                      }`}
                    >
          ...
          const amountButton = isOwn ? (
            <button
              onClick={() => onEdit(exp)}
              title="Editar"
              className="font-bold text-gray-800 dark:text-dm-text hover:text-red-600 dark:hover:text-red-400 transition underline underline-offset-2 decoration-dashed decoration-gray-400 dark:decoration-dm-faint"
            >
              R$ {exp.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </button>
          ) : (
            <span className="font-bold text-gray-800 dark:text-dm-text">
              R$ {exp.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </span>
          )
          ...
          return (
            <div key={exp.id}>
              {/* Mobile card */}
              <div className="sm:hidden p-4 bg-gray-50 dark:bg-dm-field rounded-xl">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <div className={`w-2 h-2 rounded-full shrink-0 mt-1.5 ${category?.color ?? 'bg-gray-400'}`} />
                    <p className="font-medium text-gray-800 dark:text-dm-text leading-snug">
                      {exp.description} • {date.toLocaleDateString('pt-BR')}
                    </p>
                  </div>
                  <div className="shrink-0 ml-2">{amountButton}</div>
                </div>
                <p className="text-xxs text-gray-500 dark:text-dm-muted mt-1 ml-4">
                ...
                <div className="flex items-center justify-between mt-2.5 ml-4">
                  ...
                  <div className="relative">
                    ...
                    {editingCategoryId === exp.id && (
                      <div className="absolute left-0 top-full mt-1 z-20 bg-white dark:bg-dm-card rounded-xl shadow-xl border border-gray-100 dark:border-white/[0.08] py-1 min-w-[160px]">
                        {categories.map(cat => (
                          <button
                            key={cat.id}
                            onClick={() => {
              ...
              {/* Desktop row */}
              <div className="hidden sm:flex items-center justify-between p-4 bg-gray-50 dark:bg-dm-field rounded-lg hover:bg-gray-100 dark:hover:bg-dm-hover transition">
                <div className="flex items-center gap-4 flex-1">
                  <div className={`w-2 h-2 rounded-full ${category?.color ?? 'bg-gray-400'}`} />
                  <div className="flex-1">
                    <p className="font-medium text-gray-800 dark:text-dm-text">
                      {exp.description} • {date.toLocaleDateString('pt-BR')}
                    </p>
        ...
        {expenses.length === 0 && (
          <p className="text-center text-gray-500 dark:text-dm-muted py-8">Nenhuma despesa cadastrada ainda.</p>
        )}

        {expenses.length > 0 && hasActiveFilter && filtered.length === 0 && (
          <p className="text-center text-gray-500 dark:text-dm-muted py-8">Nenhum lançamento com este filtro.</p>
        )}
      </div>
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100 dark:border-white/[0.08]">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              disabled={page === 0}
              className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-dm-field transition text-gray-500 dark:text-dm-muted hover:text-gray-700 dark:hover:text-dm-muted disabled:opacity-30 disabled:pointer-events-none"
            >
              <ChevronLeft size={16} />
            </button>
            <span className="text-sm text-gray-600 dark:text-dm-muted px-2">{page + 1} / {totalPages}</span>
            <button
              onClick={() => setPage(p => p + 1)}
