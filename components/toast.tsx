  ...
  return (
    <div className="fixed top-4 right-4 z-[70] flex flex-col gap-2 pointer-events-none">
      {toasts.map(t => (
        <div
          key={t.id}
          className={`flex items-center gap-3 bg-white dark:bg-dm-card border border-l-4 ${STYLES[t.type].bar} border-gray-100 dark:border-white/[0.08] px-4 py-3 rounded-xl shadow-lg max-w-xs pointer-events-auto`}
        >
          {STYLES[t.type].icon}
          <span className="text-sm font-medium text-gray-700 dark:text-dm-text flex-1 leading-snug">{t.msg}</span>
          <button onClick={() => dismiss(t.id)} className="ml-1 text-gray-300 hover:text-gray-500 dark:text-dm-faint dark:hover:text-dm-muted transition shrink-0">
            <X size={13} />
          </button>
        </div>
      ))}
...
export function ConfirmModal({
  ...
  return (
    <Modal open={open} onClose={onCancel} size="sm">
      <h3 className="text-lg font-bold text-gray-800 dark:text-dm-text mb-1">{title}</h3>
      {body && <p className="text-sm text-gray-500 dark:text-dm-muted mb-5">{body}</p>}
